          ...
          >
            {city.advantages.map((adv, i) => (
              <div key={i} className="bg-white rounded-2xl p-6 shadow-lg border border-slate-100 flex items-start gap-3">
                ...
                <span className="font-bold text-[#0b1a33] text-sm">{adv.title}</span>
              </div>
            ))}
          </motion.div>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {city.popularRoutes.map((route, i) => (
                <Link
                  key={i}
                  href={`/marshrut/${route.from.toLowerCase().replace(/\s+/g, "-")}-${route.to.toLowerCase().replace(/\s+/g, "-")}`}
                ...
                >
                  ...
                  <div className="text-xs text-slate-400 font-normal mt-1">{route.distance} · {route.price}</div>
                </Link>
              ))}
            </div>
          </motion.div>
