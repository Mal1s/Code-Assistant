...
import logoUvmStal from "@assets/uvmstal_logo.png?v=3";
...
export default function CityPage() {
  ...
  const city = cities[citySlug || ""] || cities["moskva"]; // если город не найден — покажет Москву
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white mb-6 md:mb-8 leading-[1.1] md:leading-[1.05]">
              ...
              В <span className="text-[#f05a28]">{city?.name?.toUpperCase() || "РОССИИ"}</span>
            </h1>
            <p className="text-lg md:text-2xl text-slate-300 mb-8 md:mb-12 max-w-2xl leading-relaxed">
              {city?.description || "Современный автопарк 2023 года выпуска. Полное страхование грузов."}
              <br />
            ...
            <motion.div {...fadeInUp}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-5">
                {[
                  ...
                  <div
                    key={item.label}
                    className="bg-slate-50 p-3 sm:p-4 md:p-5 rounded-xl flex gap-3 md:gap-4 items-center border border-slate-100 shadow-sm"
                  >
                    <div className="w-10 h-10 sm:w-11 sm:h-11 md:w-14 md:h-14 bg-[#0b1a33] text-white rounded-xl flex items-center justify-center text-lg sm:text-xl md:text-2xl shrink-0">
                      <i className={`fas fa-${item.icon}`}></i>
                    </div>
                    <div>
                      <div className="text-[10px] sm:text-[11px] md:text-sm font-black text-[#f05a28] uppercase tracking-wider">
                        {item.label}
                      </div>
                      <div className="font-bold text-[#0b1a33] text-xs sm:text-sm md:text-base">
                        {item.val}
                      </div>
                    </div>
                  </div>
        ...
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 px-4">
              {[
                {
                  label: "Адрес офиса",
                  val: "170001, г. Тверь, пр-кт Калинина, д.17, офис 316",
                  sub: "",
                  icon: "map-marker-alt",
                },
                {
